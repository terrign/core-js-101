...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const w = y1 > y2 ? y1 - y2 : y2 - y1;
  const h = x1 > x2 ? x1 - x2 : x2 - x1;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const cos = (x1 * x2 + y1 * y2) / (Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(y2 * y2 + x2 * x2));
...
function isPrime(n) {
  if (n <= 1) return false;
  if (n < 4) return true;
  if (n % 2 === 0 || n % 3 === 0 || (n % 5 === 0 && n !== 5)) return false;
 ...
 */
function toNumber(n, k) {
  if (Number.isNaN(n * 1)) return k;
  return n * 1;
}
